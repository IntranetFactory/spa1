...
function getIsValidMaxNumber(max: unknown): max is number {
  return getIsValidNumber(max) && (max as number) > 0
}
...
function getIsValidValueNumber(
  ...
  return getIsValidNumber(value) && (value as number) <= max && (value as number) >= min
}
...
function Gauge(props: GaugeProps) {
  ...
  const value = getIsValidValueNumber(valueProp, min, max)
    ? valueProp
    : getIsValidNumber(valueProp) && (valueProp as number) > max
      ? max
      : getIsValidNumber(valueProp) && (valueProp as number) < min
        ? min
        : null

  const valueText = getIsValidNumber(value)
    ? getValueText(value as number, min, max)
    : undefined
  ...
  const contextValue = React.useMemo<GaugeContextValue>(
    ...
    [
      value, valueText, max, min, state, radius, thickness, size, center,
      percentage, startAngle, endAngle, arcLength, arcCenterY, valueTextId, labelId,
    ],
  )
  ...
  return (
    <GaugeContext.Provider value={contextValue}>
      <RootPrimitive
        ...
        aria-valuemax={max}
        aria-valuemin={min}
        aria-valuenow={getIsValidNumber(value) ? (value as number) : undefined}
...
function GaugeRange(props: PathProps) {
  ...
  const {
    center, radius, startAngle, endAngle, value, max, min, state,
    thickness, arcLength, percentage,
...
function GaugeValueText(props: DivProps) {
  ...
  const ValueTextPrimitive = asChild ? SlotPrimitive : "div"
